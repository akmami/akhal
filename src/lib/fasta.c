...
// header text after '>' up to the first whitespace; caller frees
...
// takes ownership of name, freeing it if the record cannot be stored
static int flush_record(fasta_t *fa, fmap_t *h, char *name, kstring_t *seq) {
    if (!name) return AK_OK;
    if (reserve_rec(fa) != AK_OK) {
        free(name);
        return AK_ENOMEM;
    }
    ...
    if (absent) {
        ...
        ak_log(AK_LOG_WARN, "fasta", "duplicate sequence name %s", r->name);
    }
